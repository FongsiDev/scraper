...
const isChannelVerified = (vRender) => {
  const badges = vRender["ownerBadges"]
    ? vRender["ownerBadges"].map(
        (badge) => badge["metadataBadgeRenderer"]["style"],
      )
    : [];
  return (
...
const getChannelLink = (cRender) => {
  return (
    "https://www.youtube.com" +
    cRender.navigationEndpoint.browseEndpoint.canonicalBaseUrl
  );
};
...
const getGeneralData = (renderer) => {
  return {
    ...
    download: require("./Down").ytdl.bind(shareLink(renderer.videoId)),
  };
};
...
const getPlaylistVideoData = (renderer) => {
  return {
    ...
    duration_raw: renderer.lengthText
...
const getUploadDate = (vRender) => {
  return vRender["publishedTimeText"]
    ? vRender["publishedTimeText"]["simpleText"]
    : "";
};
...
exports.getPlaylistData = (item) => {
  ...
  pRender.videos.forEach((video) =>
    preview.push(getPlaylistVideoData(video.childVideoRenderer)),
  );
  ...
  return assign(
    {
      ...
      duration_raw: vRender.lengthText
        ? vRender.lengthText.simpleText
        : "00:00:00",
...
exports.isStream = (item) =>
  item.videoRenderer && !item.videoRenderer.lengthText;
