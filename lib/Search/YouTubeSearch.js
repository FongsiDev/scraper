...
const Util = require("../../utils/UtilYt.js");
...
class Scraper {
  ...
  _extractData(json) {
    ...
    if (json.richGridRenderer) {
      contents = json.richGridRenderer.contents
        .filter(
          (item) => item.richItemRenderer && item.richItemRenderer.content,
        )
  ...
  _fetch(search_query, searchType = "VIDEO", requestedLang = this._lang) {
    if (requestedLang && typeof requestedLang !== "string") {
      throw new TypeError(
        "The request language property was not a string while a valid IANA language subtag is expected.",
      );
    }
  ...
  _parseData(data) {
    ...
    for (const item of data) {
      ...
      if (Util.isVideo(item)) results.videos.push(Util.getVideoData(item));
  ...
  async search(query, options = {}) {
    const webPage = await this._fetch(
      query,
      options.searchType,
      options.language,
    );
